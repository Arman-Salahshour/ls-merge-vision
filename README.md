...
| file | what it does |
|---|---|
| `resnet20.py` | ResNet-20 for CIFAR, option-A shortcuts so every kept conv stays at `C_in ∈ {16,32,64}` |
| `chunking.py` | the whole weight↔chunk chain: normalise, compand, pad, chunk, plus the manifest that inverts it |
| `extract.py` | CLI wrapper around chunking, runs the three verification gates on one checkpoint |
| `dataset.py` | `ResZoo`, one item = one sequence of 16 chunks; and the zoo split logic |
| `transformer.py` | pre-norm transformer blocks with rotary position encoding |
| `transformer_vae.py` | the encoder/decoder pair, depth and stage conditioning, masked β-VAE loss |
| `reconstruction.py` | decode → load into a ResNet → recalibrate BatchNorm → evaluate |
| `latent_analysis.py` | t-SNE grids by depth, expert separation, posterior overlap |
| `weight_stats.py` | per-layer moments, tail mass, per-filter kurtosis, PCA spectra |
| `cifar_eval_data.py`, `constants.py` | evaluation loaders and the shared configuration |
| `phase_one_generate_model_zoo.ipynb` | trains the backbone, the five experts, and the independent runs |
...
Before the first run, edit `constants.py`: `IMG_PATH` points at an absolute path for saved
figures, and `ZOO_PATH` defaults to `./zoo_chunks`. CIFAR-100 is loaded with
...
```bash
python extract.py ./res_models/backbone_a.pt out_backbone/ \
